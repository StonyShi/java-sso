 ...
 */
public class JedisSentinelTemplate implements JedisTemplate, InitializingBean, Serializable {
    ...
    private boolean isBitCount = false;
     ...
     * @return
     */
    static int indexForByBit(int h, int length) {
        return h & (length-1);
    }
    ...
    private int doFindIndex(String key, int shardsCount){
        if(isBitCount){
            int h = hash(key);
            //key 值从 1 开始
            return (1 + indexForByBit(h, shardsCount));
        }else{
            int h = hashAbs(key);
            //key 值从 1 开始
